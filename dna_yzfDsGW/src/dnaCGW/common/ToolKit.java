...
public class ToolKit
{
    ...
    public static byte[] read(int length,InputStream is) throws Exception {
		//从输入流中读取长度为Length的字节数据
		byte[] result = new byte[length];
		int rNum = 0;
		while(rNum < result.length) {
			byte[] tt = new byte[result.length-rNum];
			int tmp = is.read(tt);//读取一定字节数存放到临时数组tt中,返回值用tmp接收;
    		if(tmp < 0){
    			break;
    		}
    		System.arraycopy(tt, 0, result, rNum, tmp);//将临时数组tt中长度为tmp的字节复制到result数组的起始位置为rNum中;
    		rNum += tmp;
		}
		return result;
	}

    public synchronized static void writeLog(LogLevel logLevel, String processName, String threadName, String eventMessage)
    {//记录日志文件
      //  System.out.println(processName + " " + threadName + " " + eventMessage);
    	...
    	try {
			...
			System.out.println("creat logger failed!");
		}
    	if(logger==null){
    		return;
    	}
    ...
    public synchronized static void writeLog(String processName, String threadName, String message, Exception eventMessage)
    {
        String exStr = "";
    ...
    public synchronized static void writeLog(String processName, String message, Exception eventMessage)
    {
        ...
        if (eventMessage != null)
        {
            exStr = "" + eventMessage.getMessage();//Exception转换为String类型
            for (StackTraceElement ste : eventMessage.getStackTrace()){//把异常压入跟踪堆栈
            	exStr = exStr + " " + ste.toString();
            }
        }
        writeLog(LogLevel.EXCEPTION, processName, Thread.currentThread().getName(), exStr);//Thread.currentThread()获取当前线程
    }
    ...
    /*调用本地资源使filename文件中key字段翻译为相应的语言*/
    public synchronized static String getPropertyFromFile(String filename, String key)
    {
    	try{
	        ResourceBundle rb = ResourceBundle.getBundle(filename);//获取文件
	        return rb.getString(key).trim();//翻译相应字段并去除空格
    	}catch(Exception e){
	...
	public static String bcdBytes2Str(byte[] bytes,boolean cut,boolean cutLeft) {
		...
		for(int i = 0;i < bytes.length;i++) {
			...
			byte h = (byte)((bytes[i]&0xf0) >>> 4);//&11110000后无符号右移4位
	...
	/**
	 * 锟斤拷锟诫：95BE5779 04DC9CF7锟斤拷锟斤拷锟斤拷锟�9 35 42 45 35 37 37 39  30 34 44 43 39 43 46 37
	 */
	...
	public static String round(String amt) {
		...
		int a = (int)Math.round(amount*100);//四舍五入
    ...
    public static String getElementValue(String elemName, Document doc) {
        ...
        if (null != doc) {
            ...
            if (null != elem && null != elem.getFirstChild()) {
                elemValue = elem.getFirstChild().getNodeValue();//返回xml子节点的值
            }
        }
        return elemValue;
    }
    ...
    //鏄惁涔辩爜
  	public static boolean isMessyCode(String str) {
  		...
  		for (int i = 0; i < temp.length(); i++) {
  			...
  			// 褰撲粠Unicode缂栫爜鍚戞煇涓瓧绗﹂泦杞崲鏃讹紝濡傛灉鍦ㄨ瀛楃闆嗕腑娌℃湁瀵瑰簲鐨勭紪鐮侊紝鍒欏緱锟�x3f锛堝嵆闂彿瀛楃?锟�
  			// 浠庡叾浠栧瓧绗﹂泦鍚慤nicode缂栫爜杞崲鏃讹紝濡傛灉杩欎釜浜岃繘鍒舵暟鍦ㄨ瀛楃闆嗕腑娌℃湁鏍囪瘑浠讳綍鐨勫瓧绗︼紝鍒欏緱鍒扮殑缁撴灉锟�xfffd
